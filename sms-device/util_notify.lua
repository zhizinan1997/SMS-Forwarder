...
local notify = {
    -- 渠道: 自定义后端 (server.js)
    ["server"] = function(msg, extra_data)
        if config.SERVER_API == nil or config.SERVER_API == "" then
            log.error("util_notify", "未配置 `config.SERVER_API`")
            return
        end

        local url = config.SERVER_API .. "/api/sms/receive"
        local header = {
        ...
        -- 构建请求体
        local body = {
            sender = extra_data and extra_data.sender or (config.DEVICE_ID or "System"),
            content = msg,
            time = extra_data and extra_data.time or os.date("%Y-%m-%d %H:%M:%S"),
            device_id = config.DEVICE_ID or "air780e_01"
        }
    ...
    -- 发送到 diy 需要改
    ["diy"] = function(msg)
        diy_url = "http://139.224.246.75:8114/api/sms/"
        local header = {
            ["Content-Type"] = "application/json; charset=utf-8"
        }
        log.info(msg)
        ...
        local json_data = json.encode(send_body)
        -- LuatOS Bug, json.encode 会将 \n 转换为 \b
        json_data = string.gsub(json_data, "\\b", "\\n")
        log.info("util_notify", "POST", diy_url)
        log.info("util_notify", "POST", send_body)
        log.info("util_notify", "POST", json_data)
        return util_http.fetch(nil, "POST", diy_url, header, json_data)
    end,
    -- 发送到 telegram
...
--- 发送通知并处理返回结果
-- @param msg string/table 消息内容
-- @param channel string 指定的渠道名称
-- @param extra_data table 额外附加的信息 (如发件人)
-- @return boolean 返回 true 表示处理完毕(成功或不可修复错误)，false 表示网络或其他原因需要重发
function util_notify.send(msg, channel, extra_data)
    log.info("util_notify.send", "正在尝试通过渠道发送:", channel)
    ...
    -- 通知内容追加更多信息 (server渠道不追加, 保持原始数据)
    local msg_to_send = msg
    if config.NOTIFY_APPEND_MORE_INFO and channel ~= "server" then
        msg_to_send = msg .. append()
    end

    -- 发送通知
    local code, headers, body = notify[channel](msg_to_send, extra_data)
    if code == nil then
...
--- [公开接口] 将新消息加入待推送队列
-- @param msg string/table 消息内容
-- @param channels table/string 指定渠道(不填则使用 config 配置)
-- @param extra_data table 附属数据
function util_notify.add(msg, channels, extra_data)
    ...
    for _, channel in ipairs(channels) do
        table.insert(msg_queue, {channel = channel, msg = msg, retry = 0, extra_data = extra_data})
    end
    sys.publish("NEW_MSG") -- 发布事件唤醒轮询协程
    log.debug("util_notify.add", "消息已入队，当前积压:", #msg_queue)
end
    ...
    while true do
        ...
        if next(msg_queue) ~= nil and mobile.status() == 1 then
            ...
            if item.retry > (config.NOTIFY_RETRY_MAX or 100) then
                ...
                result = util_notify.send(item.msg, item.channel, item.extra_data)
